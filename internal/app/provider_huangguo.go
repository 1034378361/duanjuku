...
func init() {
	// Allow operators to override the mirror list without recompiling.
	// JUKU_HUANGGUO_AI_MIRRORS: comma-separated HTTPS URLs, e.g.
	//   JUKU_HUANGGUO_AI_MIRRORS=https://mirror1.example.com,https://mirror2.example.com
	if env := os.Getenv("JUKU_HUANGGUO_AI_MIRRORS"); env != "" {
		var mirrors []string
		for _, raw := range strings.Split(env, ",") {
			raw = strings.TrimSpace(raw)
			if raw != "" {
				mirrors = append(mirrors, strings.TrimRight(raw, "/"))
			}
		}
		if len(mirrors) > 0 {
			huangguoAIBaseMirrors = mirrors
			huangguoAIBaseURL = mirrors[0]
		}
	}
}
