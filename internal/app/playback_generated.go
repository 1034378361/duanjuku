...
func playbackGeneratedArgs(media providerMedia, input, directory, processing string) []string {
	...
	return append(args, "-avoid_negative_ts", "make_zero", "-f", "hls", "-hls_time", "2", "-hls_list_size", "0",
		...
		"-hls_flags", "temp_file", "-hls_segment_filename", filepath.Join(directory, "%06d.m4s"), filepath.Join(directory, "index.m3u8"))
}
...
func (generated *playbackGenerated) index(playlist string) error {
