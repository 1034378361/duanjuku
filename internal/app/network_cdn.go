...
type cdnTransport struct {
	...
	transportKeys []string // insertion-order keys for deterministic FIFO eviction
...
func (transport *cdnTransport) pinnedTransport(host string, proxyURL *url.URL, routeKey, address string) *http.Transport {
	...
	// Evict the oldest transport (FIFO) when pool is full, not a random one.
	for len(transport.transports) >= 32 && len(transport.transportKeys) > 0 {
		oldKey := transport.transportKeys[0]
		transport.transportKeys = transport.transportKeys[1:]
		if old, ok := transport.transports[oldKey]; ok {
			old.CloseIdleConnections()
			delete(transport.transports, oldKey)
		}
	}
	transport.transports[key] = pinned
	transport.transportKeys = append(transport.transportKeys, key)
	return pinned
}
...
func (transport *cdnTransport) CloseIdleConnections() {
	...
	transport.transportKeys = transport.transportKeys[:0]
