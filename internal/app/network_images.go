...
func (transport *imageTransport) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	if err != nil && transport.fallback != nil && request.Context().Err() == nil {
		addresses, err = transport.fallback(request.Context(), host)
	}
	if err != nil {
	...
	for attempt := 0; attempt < 2; attempt++ {
		for _, address := range addresses {
			if !publicImageAddress(address) {
				return nil, errors.New("封面地址指向本机、内网或保留网络")
			}
		}
		for _, address := range addresses {
		...
		if attempt != 0 || transport.fallback == nil || len(tried) >= 4 {
			break
		}
		addresses, err = transport.fallback(request.Context(), host)
		if err != nil {
