...
type huangguoBrowserTransport struct {
	...
	clientKeys []string // insertion-order keys for deterministic LRU eviction
...
func (transport *huangguoBrowserTransport) client(request *http.Request) (browserHTTPClient, error) {
	...
	// Evict the oldest client (FIFO) when pool is full, not a random one.
	for len(transport.clients) >= 8 && len(transport.clientKeys) > 0 {
		oldKey := transport.clientKeys[0]
		transport.clientKeys = transport.clientKeys[1:]
		if old, ok := transport.clients[oldKey]; ok {
			old.CloseIdleConnections()
			delete(transport.clients, oldKey)
		}
	}
	transport.clients[key] = client
	transport.clientKeys = append(transport.clientKeys, key)
	return client, nil
}
...
func (transport *huangguoBrowserTransport) CloseIdleConnections() {
	...
	transport.clientKeys = transport.clientKeys[:0]
	transport.mu.Unlock()
