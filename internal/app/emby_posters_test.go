...
func TestEmbyPostersScanDelayRetryRestartAndCustomArtwork(t *testing.T) {
	...
	err = filepath.WalkDir(manager.document.Settings.OutputDir, func(path string, entry os.DirEntry, err error) error {
		if err == nil && !entry.IsDir() && !strings.HasSuffix(path, ".strm") && !strings.HasSuffix(path, ".nfo") && entry.Name() != ".juku-emby.json" {
			t.Error("synchronization wrote an image into the export directory", entry.Name())
		}
		return err
	})
	if err != nil {
