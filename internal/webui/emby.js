...
export function createEmby(app) {
  ...
  const posterStates = {pending: '海报待同步', waiting: '海报等待扫描', synced: '海报已同步', existing: '保留已有海报', missing: '暂无海报地址', unconfigured: '海报未连接 Emby', failed: '海报同步失败，将重试'};
  ...
  function render() {
    ...
    const imported = entries.filter(entry => entry.episodes > 0).length;
    ...
    $('embySummary').textContent = '已导入 ' + imported + ' 部 · ' + (state.episodes || 0) + ' 集' + (state.pending ? ' · 待处理 ' + state.pending + ' 部' : '');
    $('embyPosterSummary').textContent = state.settings.serverUrl ? '海报已同步 ' + (state.postersSynced || 0) + ' 部' + (state.postersPending ? ' · 等待 ' + state.postersPending + ' 部' : '') + (state.postersFailed ? ' · 失败待重试 ' + state.postersFailed + ' 部' : '') : '填写 Emby 服务器地址和 API Key 后可自动同步海报。';
    ...
    for (const entry of entries.slice(0, 100)) {
      ...
      row.append(element('strong', '', entry.title || entry.dramaId), element('span', 'small', entry.episodes + ' 集 · ' + date(entry.syncedAt)));
